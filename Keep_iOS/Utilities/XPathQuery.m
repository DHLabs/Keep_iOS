...
BOOL PerformXPathEvaluation(xmlDocPtr doc, NSString *query)
{
    ...
    NSLog(@"bool value: %d", xpathObj->boolval);
    BOOL result = xpathObj->boolval;
...
BOOL EvaluateXPathExpression(NSData *document, NSString *expression)
{
    xmlDocPtr doc;
	...
	BOOL result = PerformXPathEvaluation(doc, expression);
    xmlFreeDoc(doc);

	return result;
}

NSString *PerformXPathCalculation(xmlDocPtr doc, NSString *query)
{
    xmlXPathContextPtr xpathCtx;
    xmlXPathObjectPtr xpathObj;
    ...
    /* Evaluate xpath expression */
    xpathObj = xmlXPathEval((xmlChar *)[query cStringUsingEncoding:NSUTF8StringEncoding], xpathCtx);
    if(xpathObj == NULL) {
		NSLog(@"Unable to evaluate XPath.");
		return nil;
    }

	NSString * resultString = [NSString stringWithUTF8String:(const char*)xpathObj->stringval];
...
NSString * EvaluateXPathCalculate(NSData *document, NSString *expression)
{
    xmlDocPtr doc;
	...
	NSString * result = PerformXPathCalculation(doc, expression);
    xmlFreeDoc(doc);
